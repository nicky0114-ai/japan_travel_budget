    ...
    childTxs.forEach(tx => {
        ...
        item.innerHTML = `
            ...
            <div class="history-item-right">
                ...
                <button class="card-icon-btn" style="margin-right: 6px;" onclick="editTransaction('${tx.id}', false)">✏️</button>
                <button class="delete-item-btn" onclick="deleteTransaction('${tx.id}', false)">🗑️</button>
            </div>
        `;
    ...
    filtered.forEach(tx => {
        ...
        item.innerHTML = `
            ...
            <div class="history-item-right">
                ...
                <button class="card-icon-btn" style="margin-right: 6px;" onclick="editTransaction('${tx.id}', true)">✏️</button>
                <button class="delete-item-btn" onclick="deleteTransaction('${tx.id}', true)">🗑️</button>
            </div>
        `;
        list.appendChild(item);
    });
...
// 編輯記帳
function editTransaction(id, isParentView) {
    try {
        ...
        document.getElementById("edit-tx-is-parent-view").value = isParentView ? "true" : "false";
        
        // 渲染成員下拉選單
        const userSelect = document.getElementById("edit-tx-user");
        userSelect.innerHTML = "";
        state.members.forEach(m => {
            const opt = document.createElement("option");
            opt.value = m.id;
            opt.text = `${m.avatar} ${m.name}`;
            if (m.id === tx.userId) opt.selected = true;
            userSelect.appendChild(opt);
        });
        
        // 設定日期時間 (轉為 YYYY-MM-DDThh:mm，防範無效或缺少 timestamp 欄位)
        ...
        document.getElementById("edit-tx-date").value = localISOTime;
        
        document.getElementById("edit-tx-jpy").value = tx.jpy;
        
        // 類別選擇
        const catSelect = document.getElementById("edit-tx-category");
        catSelect.value = tx.category || "飲食";
        
        // 支付方式
        const paySelect = document.getElementById("edit-tx-paymethod");
        paySelect.value = tx.payMethod || "cash";
        
        // 刷卡卡別
        const cardSelect = document.getElementById("edit-tx-card");
        cardSelect.innerHTML = '<option value="">-- 選擇信用卡 --</option>';
        state.creditCards.forEach(c => {
            const opt = document.createElement("option");
            opt.value = c.name;
            opt.text = c.name;
            if (c.name === tx.cardName) opt.selected = true;
            cardSelect.appendChild(opt);
        });
        
        const cardGroup = document.getElementById("edit-tx-card-group");
        if (tx.payMethod === "card") {
...
function handleEditPayMethodChange(value) {
    const cardGroup = document.getElementById("edit-tx-card-group");
    if (value === "card") {
        cardGroup.classList.remove("hidden");
    } else {
        cardGroup.classList.add("hidden");
        document.getElementById("edit-tx-card").value = "";
    }
}
...
function saveTransactionEdit() {
    ...
    const isParentView = document.getElementById("edit-tx-is-parent-view").value === "true";
    
    const txIndex = state.transactions.findIndex(t => t.id === id);
    if (txIndex === -1) return;
    
    const userId = document.getElementById("edit-tx-user").value;
    const member = state.members.find(m => m.id === userId);
    const dateVal = document.getElementById("edit-tx-date").value;
    const jpyVal = parseFloat(document.getElementById("edit-tx-jpy").value) || 0;
    const category = document.getElementById("edit-tx-category").value;
    const payMethod = document.getElementById("edit-tx-paymethod").value;
    const cardName = document.getElementById("edit-tx-card").value;
    ...
    // 計算台幣
    const twdVal = Math.round(jpyVal * state.exchangeRate);
    
    // 更新交易
    state.transactions[txIndex] = {
        ...state.transactions[txIndex],
        userId: userId,
        userName: member ? member.name : "未知",
        userAvatar: member ? member.avatar : "❓",
